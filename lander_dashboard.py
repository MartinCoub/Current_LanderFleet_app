...
# ────────────── argparse setup to open .pkl data ──────────────
parser = argparse.ArgumentParser(description="Lander Dashboard: specify the .pkl file containing the data.")
parser.add_argument( "--pkl_file",type=str,required=True, help="Filename of the .pkl to load, e.g. 'Cluster_Datasets_Landers_V2.pkl'")
args = parser.parse_args()
pkl_file = args.pkl_file
path_save= os.path.join(path_data, '7_Cluster_ds_lander')
path_pkl = os.path.join(path_save, pkl_file)
if not os.path.isfile(path_pkl):
    raise FileNotFoundError(f"Specified .pkl does not exist: {pkl_file}")
